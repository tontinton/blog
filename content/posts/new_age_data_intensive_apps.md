...
In `Designing Data-Intensive Applications`, Martin Kleppmann suggests that all data applications follow a similar pattern. Their goal is to read data, run some transformation on it, and store the result somewhere, all have a faster way to read that data later.
...
Ok, we're done with the theory, let's look at some real open-source data applications that have explicitly decided to use an object storage.
