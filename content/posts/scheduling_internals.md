...
I assume proficiency in reading and understanding code at an intermediate level, so don't stress over small details you don't understand, try to get the bigger picture.
...
The first problem with threads is that each thread you create allocates a **ton** of stack memory by default, on linux it's 10MB and on windows it's 1MB.

So if you have a linux server with 8GB of RAM, you have a limit of ~800 clients you can handle concurrently by default.

This can be fixed by changing the stack size using `pthread_attr_setstacksize` for example, but the size must be a multiple of the system page size, which might still be too large for our application needs.

The second problem we need to fix when using OS threads is to change some limits:
...
The fourth problem is that the stack size is static, we can't grow / shrink the thread's stack size at runtime.

Because of all these problems, threads should not be your go-to solution for running a lot of tasks concurrently (especially for I/O bound tasks like in `echod`). Running a thread per client is famously known as <a href="http://kegel.com/c10k.html">The C10K Problem</a>.
...
In real non-preemptive (also called cooperative) systems, the runtime should yield when it knows that the CPU has nothing useful to do anymore in the current task, for example waiting on I/O. They do that by registering for I/O and move the task to a different queue that holds idle tasks (which the scheduler skips from running). Once there's I/O, they move the task from the idle queue back to the regular queue for execution. This can be done for example by integrating with an event loop.
...
This is exactly how Go made their scheduler preemptive in version 1.14. By sending signals periodically from their monitoring thread (<a href="https://sobyte.net/post/2021-12/golang-sysmon/">runtime.sysmon</a>) to the scheduler threads running goroutines.
